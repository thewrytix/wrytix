...
const create = async (req, res) => {
    try {
        ...
        if (!name || !editor) {
            await logAction(req.session.user?.username || 'anonymous', 'category-create-failed', 'system', {
                reason: 'Name or editor missing'
            });
        ...
        if (!editorUser || editorUser.role !== 'editor') {
            await logAction(req.session.user?.username || 'anonymous', 'category-create-failed', 'system', {
                reason: 'Invalid or non-editor user'
            });
        ...
        await logAction(req.session.user?.username || 'anonymous', 'category-created', category.id, {
            name: category.name,
            editor
        });
        ...
        await logAction(req.session.user?.username || 'anonymous', 'category-create-error', 'system', {
            error: error.message
        });
...
const update = async (req, res) => {
    try {
        const { name, editor, authors } = req.body;
        if (!name || !editor) {
            await logAction(req.session.user?.username || 'anonymous', 'category-update-failed', req.params.id, {
                reason: 'Name or editor missing'
            });
            return res.status(400).json({ error: 'Name and editor are required' });
        }
        ...
        await logAction(req.session.user?.username || 'anonymous', 'category-updated', updatedCategory.id, {
            changes: Object.keys(req.body)
        });

        res.json({ message: 'Category updated', category: updatedCategory });
    } catch (err) {
        await logAction(req.session.user?.username || 'anonymous', 'category-update-error', req.params.id, {
            error: err.message
        });
...
const deleteCategory = async (req, res) => {
    try {
        const category = await Category.findOne({ id: req.params.id }).lean();
        if (!category) {
            await logAction(req.session.user?.username || 'anonymous', 'category-delete-failed', req.params.id, {
                reason: 'Not found'
            });
            return res.status(404).json({ error: 'Category not found' });
        }
        ...
        await logAction(req.session.user?.username || 'anonymous', 'category-deleted', category.id, {
            name: category.name,
        ...
        await logAction(req.session.user?.username || 'anonymous', 'category-delete-error', req.params.id, {
            error: err.message
        });
        res.status(500).json({ error: 'Server error' });
    }
